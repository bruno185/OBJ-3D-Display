...
PRG = "poccompare"  # Program name without extension
lang = ".cc"
...
def main():
    ...
    # Compile the program
    if not run_command(f"iix compile +D {PRG}{lang}", "Compiling source"):
        sys.exit(1)
    
    # Link the program
    if not run_command(f"iix -DKeepType=S16 link +L {PRG} keep={PRG} > out.map", "Linking program"):
        sys.exit(1)
