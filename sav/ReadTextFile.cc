...
// Fonction pour transformer les coordonnees dans le systeme de l'observateur
...
// Fonction pour traiter le modele avec les parametres
...
int main() {
    ...
    float angle_h, angle_v, distance, angle_w;
    
    printf("Programme de lecture de fichier 3D\n");
    printf("===================================\n\n");
    
    // Allouer de la memoire pour le tableau de vertices
    vertices = (Vertex3D*)malloc(MAX_VERTICES * sizeof(Vertex3D));
    if (vertices == NULL) {
        printf("Erreur: Impossible d'allouer la memoire pour %d vertices\n", MAX_VERTICES);
        printf("Press any key to quit...\n");
        keypress();
        return 1;
    }
    
    // Allouer de la memoire pour le tableau de faces
    faces = (Face3D*)malloc(MAX_FACES * sizeof(Face3D));
    if (faces == NULL) {
        printf("Erreur: Impossible d'allouer la memoire pour %d faces\n", MAX_FACES);
        printf("Press any key to quit...\n");
        keypress();
        free(vertices);
        return 1;
    ...
    if (fgets(filename, sizeof(filename), stdin) != NULL) {
        // Enlever le caractere de nouvelle ligne si present
    ...
    // Lire les vertices du fichier
    vertex_count = readVertices(filename, vertices, MAX_VERTICES);
    ...
    // Lire les faces du fichier
    face_count = readFaces(filename, faces, MAX_FACES);
    
    // Verifier si la lecture des faces a reussi
    if (face_count < 0) {
        printf("\nAvertissement: Impossible de lire les faces\n");
        face_count = 0;  // Continuer sans les faces
    }
    
    // Demander les parametres de l'observateur
    printf("\nParametres de l'observateur:\n");
    printf("============================\n");
    ...
    // Appliquer la transformation des coordonnees
    transformToObserver(vertices, vertex_count, angle_h, angle_v, distance);
    
    // Projeter sur l'ecran 2D
    projectTo2D(vertices, vertex_count, angle_w);
    ...
    if (vertex_count > 0) {
        ...
        int i;
        for (i = 0; i < vertex_count; i++) {
            if (vertices[i].x2d >= 0 && vertices[i].y2d >= 0) {
                printf("  Vertex %3d: (%.2f,%.2f,%.2f) -> (%.2f,%.2f,%.2f) -> (%d,%d)\n", 
                       i + 1, 
                       vertices[i].x, vertices[i].y, vertices[i].z,
                       vertices[i].xo, vertices[i].yo, vertices[i].zo,
                       vertices[i].x2d, vertices[i].y2d);
            } else {
                printf("  Vertex %3d: (%.2f,%.2f,%.2f) -> (%.2f,%.2f,%.2f) -> (invisible)\n", 
                       i + 1, 
    ...
    if (face_count > 0) {
        ...
        int i, j;
        for (i = 0; i < face_count; i++) {
            printf("  Face %3d (%d vertices): ", i + 1, faces[i].vertex_count);
            for (j = 0; j < faces[i].vertex_count; j++) {
                printf("%d", faces[i].vertex_indices[j]);
                if (j < faces[i].vertex_count - 1) printf("-");
            }
            printf("\n");
        }
        
        // Dessiner les polygones avec QuickDraw
        drawPolygons(vertices, faces, face_count);
    }
    
    // Boucle pour permettre de recalculer avec de nouveaux angles
    while (1) {
        ...
        if (fgets(input, sizeof(input), stdin) != NULL) {
            if (input[0] == 'n' || input[0] == 'N') {
                ...
                // Demander les nouveaux parametres
                ...
                // Recalculer la transformation et la projection
                transformToObserver(vertices, vertex_count, angle_h, angle_v, distance);
                projectTo2D(vertices, vertex_count, angle_w);
    ...
    // Attendre 1 seconde avant de quitter
    delay(1);
